...
public class Inscripción_Grupo implements IModule {
    ...
    public String getResponse(JSONObject jobject) {
        ...
        try {
            ...
            insertar_persona(cedula, nombre, url);
            int id = extraer_partido();
            insertar_candidato(cedula, id);

            return "{\"Inscripción_Grupo\":\"exit\"}";
        } catch (SQLException ex) {
            Logger.getLogger(Inscripción_Grupo.class.getName()).log(Level.SEVERE, null, ex);
        ...
        return "{\"Inscripción_Grupo\":\"failed\"}";
    }
    ...
    public void insertar_persona(String cedula, String nombre, String url) {
        try {
            statement = (PreparedStatement) connection.prepareStatement("insert into persona(cedula_persona,nombre,url_foto) values(?,?,?)");
